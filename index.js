...
app.get("/", (req, res, next) => {
  res.render("formulario_InteresCompuesto");
});
...
app.post("/interes", (req, res, next) => {
  ...
  const { deposito, tasaInteres, periodo } = req.body;

  const saldoFinals = calcularInteresCompuesto(deposito, tasaInteres, periodo);

  res.render("resultado_interes", { saldoFinals });
});
